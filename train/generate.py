...
def generate(args):
    ...
    required_files = total_positions // positions_per_file
    ...
    if model_gens:
        ...
        gen = re.findall("[0-9]+", latest_model)[0] + 1

    # Load the model :
    model = OnitamaNet(config).to(device)

    if latest_model:
        print(f"Found model for v{gen} generation. Loading from {latest_model}.")
    ...
    data_current_gen_folder = os.path.join(data_folder, f"v{gen}")
    os.makedirs(data_current_gen_folder, exist_ok=True)

    data_current_gen_files = sorted(os.listdir(data_current_gen_folder), key = lambda f : re.findall("[0-9]+", f)[0])
    
    last_file = data_current_gen_files[-1] if data_current_gen_files else None

    if last_file:
        last_file_index = int(last_file.split("_")[-1].split(".")[0])
        if last_file_index == required_files:
    ...
    data = []
    data_path = os.path.join(data_current_gen_folder, f"positions_{new_file_index}.pt")


    mcts = MCTS(model, mcts_config, device)

    positions = new_file_index * positions_per_file
    ...
    if data:
        remaining = total_positions - new_file_index*positions_per_file
